...
#define RQ_DEBUGGING_H
    ...
    //            error("timestamp > MAX_NUM_RQ_IN_EXECUTION");
        ...
        inline void DEBUG_RECORD_RQ_CHECKSUM(const int tid, const long long timestamp, K const * const rqResult, const int len) {
            ...
            if (timestamp < 5) {
                cout<<"timestamp="<<timestamp<<" len="<<len<<endl; ///////////////////////////////////////////
                cout<<endl;
            }
#endif
            for (int i=0;i<len;++i) {
...
#ifdef DEBUG_RQ_PROVIDER_METRICS
                    if (timestamp < 5) {
                        if (threadUpdateChecksum[tid][timestamp]) cout<<"tid="<<tid<<" timestamp="<<timestamp<<" threadUpdateChecksum="<<threadUpdateChecksum[tid][timestamp]<<endl; //////////////////////////////////////////////
                        if (threadRQChecksum[tid][timestamp]) cout<<"tid="<<tid<<" timestamp="<<timestamp<<" threadRQChecksum="<<threadRQChecksum[tid][timestamp]<<endl; //////////////////////////////////////////////
                    }
#endif
                }
#ifdef DEBUG_RQ_PROVIDER_METRICS
                if (timestamp < 5) {
                    if (updateChecksum[timestamp]) cout<<"updateChecksum[timestamp="<<timestamp<<"]="<<updateChecksum[timestamp]<<endl; ////////////////////////////////////////////////
                    if (rqChecksum[timestamp]) cout<<"rqChecksum[timestamp="<<timestamp<<"]="<<rqChecksum[timestamp]<<endl; ////////////////////////////////////////////////
                }
#endif
